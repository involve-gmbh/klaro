...
const ContextualConsentNotice = ({
    ...
    const accept = () => {
        ...
        if (manager.confirmed) {
            // we permanently save the consent state
            manager.saveConsents('contextual-accept');
            manager.applyConsents(false, true, service.name);
        } // we only temporarily accept this
        else manager.applyConsents(false, true, service.name);
    };
    const acceptOnce = () => {
        manager.updateConsent(service.name, true);
        manager.applyConsents(false, true, service.name);
    ...
    const title =
        tt(service.translations || {}, lang, 'zz', ['!', 'title']) ||
