...
export const Demo = ({ t: ttt, config }) => {
    ...
    const [lang, setLang] = useState(
        config.languages.length > 0 ? config.languages[0] : 'en'
    );
    ...
    const tt = (...args) =>
        t(trans, lang, config.fallbackLang || 'zz', ...args);
    const languages = config.languages.map((language) => (
        <option key={language} value={language}>
            {ttt(['languages', language])} ({language})
        </option>
    ));
    const testOnSite = () => {
            ...
            <form onSubmit={testOnSite}>
                <div className="cm-config-controls">
                    <BaseRetractingLabelInput
                        value={siteUrl}
                        onChange={setSiteUrl}
                        label={ttt(['demo', 'testOnSite', 'label'])}
                    />
                    <button
                        className="cm-control-button cm-success"
                        onClick={(e) => {
                            e.preventDefault();
                            testOnSite();
                        }}
