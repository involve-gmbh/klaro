...
export const TranslationsForKey = ({
    hintKey,
    translationKey,
    noDefault,
    onChange,
    ...
    */
    ...
    const description = t(['translations', ...hintKey, 'description'], {
        name: name,
    });
    const items = allLanguages.map((language) => {
        ...
        const changeValue = (v) => {
            ...
            } else onChange(language, v);
        };
        const label = t(
            [
                'translations',
                ...hintKey,
                language === 'zz' ? 'defaultLabel' : 'label',
            ],
            { name: name, language: t(['languages', language]) }
        );
        return (
            <li key={language}>
                <span className="cm-lang">
                    {language !== 'zz' ? language : '_'}
                </span>
    ...
    return (
        <div className="cm-translations-for-key">
...
export const ServiceTranslations = ({ t, config, updateConfig }) => {
    const updateDescription = (service, language, value) => {
        updateConfig(
            ['services', service._id, 'translations', language, 'description'],
            value
        );
    };
    const updateTitle = (service, language, value) => {
        updateConfig(
            ['services', service._id, 'translations', language, 'title'],
            value
        );
    };
    const serviceTranslations = config.services.map((service) => (
        <React.Fragment key={service.name}>
            <h3>{service.name}</h3>
            <TranslationsForKey
                onChange={(language, value) =>
                    updateTitle(service, language, value)
                }
                t={t}
            ...
            <TranslationsForKey
                onChange={(language, value) =>
                    updateDescription(service, language, value)
                }
                t={t}
    ...
    return (
        <React.Fragment>
            {(serviceTranslations.length > 0 && serviceTranslations) || (
...
export const PurposeTranslations = ({ t, config, updateConfig }) => {
    ...
    config.services.forEach((service) =>
        service.purposes.forEach((purpose) => purposes.add(purpose))
    );
    const updateDescription = (purpose, language, value) => {
        updateConfig(
            ['translations', language, 'purposes', purpose, 'description'],
            value
        );
    };
    const updateTitle = (purpose, language, value) => {
        updateConfig(
            ['translations', language, 'purposes', purpose, 'title'],
            value
        );
    };
    const purposeTranslations = Array.from(purposes.keys()).map((purpose) => (
        <React.Fragment key={purpose}>
            <h3>{purpose}</h3>
            <TranslationsForKey
                t={t}
                onChange={(language, value) =>
                    updateTitle(purpose, language, value)
                }
            ...
            <TranslationsForKey
                t={t}
                onChange={(language, value) =>
                    updateDescription(purpose, language, value)
                }
    ...
    return (
        <React.Fragment>
            {(purposeTranslations.length > 0 && purposeTranslations) || (
                <p className="cm-section-description">
                    {t(['translations', 'noTranslations'])}
                </p>
            )}
...
export const UITranslations = ({ t, config, updateConfig }) => {
    const translationsFor = (translations, parentKey) => {
        ...
        for (const [k, v] of Object.entries(translations)) {
            ...
            items.push(
                <div key={key.join('.')} className="cm-key-translations">
                    {content}
                </div>
            );
        }
...
export const Translations = ({ t, state, setState, config, updateConfig }) => {
    ...
    */
    ...
    const tabs = Array.from(Object.entries(components)).map(([k, v]) => (
        <Tab
            active={k === state.tab}
            onClick={() => setState({ tab: k })}
            key={k}
        >
            {t(['translations', 'headers', k])}
        </Tab>
    ));
    return (
        <React.Fragment>
