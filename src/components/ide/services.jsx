...
export const ServiceItem = ({ t, service, onClick, updateConfig }) => (
    <ListItem onClick={() => onClick(service)} isCard key={service._id}>
        <ListColumn size="md">
        ...
        <ListColumn size="icon">
            <DropdownMenu>
                <MenuItem
                    onClick={() =>
                        updateConfig(['services', service._id], null)
                    }
                >
                    {t(['services', 'delete'])}
                </MenuItem>
            ...
            {(services.length > 0 && (
                <List className="cm-service-list">
                    <ListHeader>
                        ...
                        <ListColumn size="icon">{t(['menu'])}</ListColumn>
                    </ListHeader>
                    {services}
                </List>
            )) || (
                <p className="cm-no-services">
...
export const ServiceDetails = ({
    ...
    if (service === undefined) return <div />;
    return (
...
export const ServiceConfig = ({
    ...
    const formControls = Spec.serviceConfig.map((serviceField) => {
        ...
        const updateServiceConfig = (k, v) => {
            ...
            if (k[0] === 'name') updateServiceName(v);
        };
        return (
            <ClassName
                disabled={disabled}
    ...
    return (
        <React.Fragment>
            <fieldset className="cm-service-fields" disabled={disabled}>
                <h2>
                    <a onClick={unsetService}>
                        {t(['services', 'title'])} &rsaquo;
                    </a>{' '}
                    {service.name}
                </h2>
                {formControls}
...
export const Services = ({
    ...
    if (services !== undefined)
        newServices = services.filter(
            (service) =>
                config.services.find(
                    (configService) =>
                        configService.name === service.name ||
                        configService.id === service.id
                ) === undefined
        );
    ...
    if (service !== undefined) {
        component = (
            <ServiceDetails
                ...
                t={t}
                updateConfig={updateConfig}
                service={config.services.find((ap) => ap.name === service)}
            />
        );
    } else {
        component = (
