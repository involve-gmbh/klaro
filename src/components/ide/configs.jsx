...
export const ConfigItem = ({ t, config, onConfigAction, onClick }) => (
    <ListItem onClick={() => onClick(config)} isCard key={config.name}>
        <ListColumn size="icon cm-status">
            <span
                title={config.status}
                className={'cm-status-is-' + config.status}
            >
...
export const ConfigList = ({
    ...
    return (
        <List className="cm-config-list">
            <ListHeader>
                <ListColumn size="icon">{t(['configs', 'status'])}</ListColumn>
                <ListColumn size="lg">{t(['configs', 'name'])}</ListColumn>
