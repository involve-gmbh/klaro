...
export const ServiceSelect = ({
    ...
    const serviceTitle = (service) =>
        getValue(service.spec.translations || {}, t.lang, ['title']) ||
        getValue(service.spec.translations || {}, 'zz', ['title']) ||
        asTitle(service.name);
    const generateCandidates = (services) =>
        services
            .sort((a, b) => (serviceTitle(a) > serviceTitle(b) ? 1 : -1))
            .map((service) => ({
                service: service,
                name: service.name,
                value: serviceTitle(service),
            }));
    const [candidates, setCandidates] = useState(generateCandidates(services));

    const searchServices = (query) => {
        if (!query) return services;
        let ms = services.filter((service) =>
            serviceTitle(service).toLowerCase().includes(query.toLowerCase())
        );
        return ms;
    };
    ...
    return (
        <div className="cm-service-select">
            <SearchSelect
                search={search}
                onSelect={createService}
