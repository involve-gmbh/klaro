...
export const PurposeSelect = ({
    ...
    const generateInitialPurposes = () =>
        Array.from(Object.entries(purposes))
            .filter(([k]) => !existingPurposes.has(k))
            .map(([k, v]) => ({
                name: k,
                ...
                value:
                    t.lang === 'en'
                        ? `${v.title.en}`
                        : `${v.title.en} - ${t(['purposes', k, 'title'])}`,
            }));
    const [candidates, setCandidates] = useState(generateInitialPurposes());
    const updateSearch = (value) => {
        const candidatePurposes = Array.from(Object.entries(purposes)).filter(
            ([k, v]) =>
        ...
        let candidates = candidatePurposes.map((cl) => ({
            name: cl[0],
            description: t(['purposes', cl[0], 'description']),
            value: `${cl[1].title.en} - ${t(['purposes', cl[0], 'title'])}`,
        }));
        if (candidates.length > 10) candidates = [];
        if (value !== '')
            candidates.push({
                name: value,
                description: t(['purpose', 'descriptionNotice']),
                value: `${value} (${t(['purpose', 'add'])})`,
            });
        ...
        <li key={purpose}>
            ...
            <a
                onClick={() => {
                    ...
                    updateConfig(
                        [field.name],
                        config[field.name].filter((lang) => lang !== purpose)
                    );
                }}
            >
                &#10540;
    ...
    return (
        <div className="cm-purpose-select">
            <ul className="cm-purposes">{purposeItems}</ul>
            <SearchSelect
                disabled={disabled}
                search={search}
