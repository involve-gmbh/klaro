...
export const LanguageSelect = ({
    ...
    const generateInitialCandidates = () =>
        Array.from(Object.entries(languages))
            .filter(([k]) => !config[field.name].includes(k))
            .map(([k, v]) => ({ name: k, value: `${v.en} - ${v[k]} (${k})` }));
    ...
    const updateSearch = (value) => {
        const candidateLanguages = Array.from(Object.entries(languages)).filter(
            ([k, v]) =>
                !existingLanguages.has(k) &&
                (v[k].toLowerCase().includes(search.toLowerCase()) ||
                    v.en.toLowerCase().includes(search.toLowerCase()))
        );
        let candidates = candidateLanguages.map((cl) => ({
            name: cl[0],
            value: `${cl[1].en} - ${cl[1][cl[0]]} (${cl[0]})`,
        }));
        ...
        <li key={language}>
            ...
            <a
                onClick={() => {
                    updateConfig(
                        [field.name],
                        config[field.name].filter((lang) => lang !== language)
                    );
                    setUpdated(true);
                }}
            >
    ...
    return (
        <div className="cm-language-select">
            <ul className="cm-languages">{languageItems}</ul>
            <SearchSelect
                disabled={disabled}
                search={search}
