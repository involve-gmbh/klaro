...
export const ThemesSelect = ({
    ...
    const generateInitialThemes = () =>
        Array.from(Object.entries(themes))
            .filter(([k]) => !existingThemes.has(k))
            .map(([k, v]) => ({
                name: k,
                ...
                value:
                    t.lang === 'en'
                        ? `${v.title.en}`
                        : `${v.title.en} - ${t(['themes', k, 'title'])}`,
            }));
    const [candidates, setCandidates] = useState(generateInitialThemes());
    const updateSearch = (value) => {
        const candidateThemes = Array.from(Object.entries(themes)).filter(
            ([k, v]) =>
        ...
        let candidates = candidateThemes.map((cl) => ({
            name: cl[0],
            description: t(['themes', cl[0], 'description']),
            value: `${t(['themes', cl[0], 'title'])}`,
        }));
        ...
        <li key={theme}>
            ...
            <a
                onClick={() => {
                    ...
                    updateConfig(
                        [field.name],
                        config[field.name].filter((th) => th !== theme)
                    );
                }}
            >
                &#10540;
    ...
    return (
        <div className="cm-theme-select">
            <ul className="cm-themes">{themeItems}</ul>
            <SearchSelect
                disabled={disabled}
                search={search}
