...
export const PurposeOrder = ({ t, config, updateConfig }) => {
    ...
    const sortedPurposes = Array.from(purposes.values()).sort(
        (a, b) => purposeOrder.indexOf(a) - purposeOrder.indexOf(b)
    );
    purposes.forEach((purpose) => {
        if (purposeOrder.indexOf(purpose) === -1) purposeOrder.push(purpose);
    });
    ...
    const purposeItems = sortedPurposes.map((purpose) => {
        const value = getValue(config.translations, t.lang, [
            'purposes',
            purpose,
            'title',
        ]);
        const fallbackValue = getFallbackValue(t.tv, t.lang, [
            'purposes',
            purpose,
            'title',
        ]);
        return (
            <li key={purpose}>
                <span className="cm-buttons">
                    <a className="cm-btn" onClick={() => move(purpose, -1)}>
                        &uarr;
                    </a>
                    <a className="cm-btn" onClick={() => move(purpose, 1)}>
                        &darr;
    ...
    return (
        <div className="cm-purpose-order">
