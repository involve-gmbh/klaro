...
export const JSONConfig = ({ t, config, updateConfig }) => {
    ...
    return (
        <div className="cm-json">
            <h3 className="cm-space-lg">{t(['json', 'importExport'])}</h3>
            {error && <p className="cm-message cm-error">{error}</p>}
            {message && <p className="cm-message cm-success">{message}</p>}
            <form className="cm-config-controls">
                <fieldset>
                    <label htmlFor="cm-file-import" className="cm-upload-label">
                        <a className="cm-control-button cm-success">
                            &#8613; {t(['json', 'import'])}
                        </a>
                    </label>
                    <input
                    ...
                    <a
                        ...
                        href={url}
                        className="cm-control-button cm-primary"
                    >
                        &#8615; {t(['json', 'export'])}
                    </a>
                </fieldset>
