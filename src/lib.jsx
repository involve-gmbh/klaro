...
export function render(config, opts) {
    ...
    const tt = (...args) =>
        t(configTranslations, lang, config.fallbackLang || 'zz', ...args);
    const app = reactRender(
        <App
            t={tt}
            lang={lang}
            manager={manager}
        ...
        for (const element of elements) {
            ...
            if (element.tagName === 'IFRAME' || element.tagName === 'DIV') {
                ...
                if (placeholderElement === null) {
                    ...
                    applyDataset(
                        { type: 'placeholder', name: service.name },
                        placeholderElement
                    );
                ...
                if (
                    ds['modified-by-klaro'] === undefined &&
...
function getKlaroId(script) {
    ...
    const regexMatch = /.*\/privacy-managers\/([a-f0-9]+)\/klaro.*\.js/.exec(
        script.src
    );
...
function getHashParams() {
    return new Map(
        decodeURI(location.hash.slice(1))
            .split('&')
            .map((kv) => kv.split('='))
            .map((kv) => (kv.length === 1 ? [kv[0], true] : kv))
    );
}
...
export function validateConfig(config) {
    ...
    if (
        validatedConfig.apps !== undefined &&
        validatedConfig.services === undefined
    ) {
    ...
    if (validatedConfig.translations !== undefined) {
        if (
            validatedConfig.translations.apps !== undefined &&
            validatedConfig.services === undefined
        ) {
            validatedConfig.translations.services =
                validatedConfig.translations.apps;
            console.warn(
...
export function setup(config) {
    ...
    const initialize = (opts) => {
        ...
        if (
            !defaultConfig.noAutoLoad &&
            (!defaultConfig.testing || fullOpts.testing)
        )
    ...
    if (config !== undefined) {
        ...
        if (klaroId !== null) {
            ...
            if (window.klaroApiConfigs !== undefined) {
                ...
                const config = window.klaroApiConfigs.find(
                    (config) =>
                        config.name === klaroConfigName &&
                        (config.status === 'active' || testing)
                );
                ...
                api.loadConfig(klaroConfigName)
                    .then((config) => {
            ...
            const configName =
                script.getAttribute('data-klaro-config') || 'klaroConfig';
            defaultConfig = window[configName];
            if (defaultConfig !== undefined) doOnceLoaded(() => initialize({}));
        }
    }
...
export function version() {
    // we remove the 'v'
    if (VERSION[0] === 'v') return VERSION.slice(1);
    return VERSION;
}
