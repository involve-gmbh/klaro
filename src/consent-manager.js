...
export default class ConsentManager {
    constructor(config, store, auxiliaryStore) {
        ...
        else this.store = new stores[this.storageMethod](this);

        // we fall back to the cookie-based store if the store is undefined
        if (this.store === undefined) this.store = stores['cookie'];

        if (auxiliaryStore !== undefined) this.auxiliaryStore = auxiliaryStore;
        else this.auxiliaryStore = new SessionStorageStore(this);
    ...
    getService(name) {
        const matchingServices = this.config.services.filter(
            (service) => service.name === name
        );
        if (matchingServices.length > 0) return matchingServices[0];
        return undefined;
    }
    ...
    applyConsents(dryRun, interactive, serviceName) {
        ...
        for (let i = 0; i < this.config.services.length; i++) {
            ...
            const vars = service.vars || {};
            const optOut =
                service.optOut !== undefined
                    ? service.optOut
                    : this.config.optOut || false;
            const required =
                service.required !== undefined
                    ? service.required
            ...
            const consent =
                (this.getConsent(service.name) && confirmed) || required;
            const handlerOpts = {
                service: service,
                config: this.config,
                vars: vars,
            ...
            executeHandler(
                consent ? service.onAccept : service.onDecline,
                handlerOpts
            );
        ...
        if (service.cookies !== undefined && service.cookies.length > 0) {
            ...
            for (let i = 0; i < service.cookies.length; i++) {
                ...
                if (cookiePattern instanceof Array) {
                    ...
                    cookiePattern instanceof Object &&
